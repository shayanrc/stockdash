...
- **Ingestion scripts**:
  - `update.py`: Reads stock symbols from DuckDB `universe_stocks` (once populated), downloads per-symbol history to `data/cache/price_history/<SYMBOL>.csv`. CLI: `--db-file`, `--exchange`, `--delay`, `--symbol` (single-stock mode).
...
- **Update stocks CSVs (universe)**: `python update.py --db-file data/db/stock.duckdb --exchange NSE --delay 2`
- **Update a single stock CSV**: `python update.py --db-file data/db/stock.duckdb --symbol RELIANCE`
...
- `update.py`: Batch or single-stock downloader (respects delay between requests).
